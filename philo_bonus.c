...
void	philo_routine(t_philo *philo, t_default *def, t_sems *locks)
{
	...
	while (i < def->number_philo)
	{
		...
		while(1)
		{
			...
			printf("%l %d has taken a first fork", philo->id);
			sem_wait(locks->forks);
			printf(philo, "has taken a second fork", locks);
			sem_wait(locks->last_meal_check);
			philo->last_meal = get_current_time();
			sem_post(locks->last_meal_check);
			printf(philo, "is eating", locks);
			usleep(def->time_to_eat * 1000);
			sem_post(locks->forks);
			sem_post(locks->forks);
			philo->meals++;
			printf(philo, "is sleeping", locks);
			usleep(def->time_to_sleep * 1000);
			printf(philo, "is thinking", locks);
		}
		i++;
	}
}
...
int main(int ac,char **av)
{
	...
	if (init_sems(&philo, &locks, def->number_philo) == 1)
		return (1);
	while(i < def->number_philo)
	{
		pid = fork();
